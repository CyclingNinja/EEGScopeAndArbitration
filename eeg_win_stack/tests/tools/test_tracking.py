"""Tests for cross-stage MLflow run threading (eeg_win_stack/tools/tracking.py)."""
...
class TestConfigAccessors:
    ...
    def test_experiment_name_bare_when_azure(self, cfg):
        ...
        assert tracking.experiment_name(cfg) == "eeg_win_stack"
...
class TestStartRun:
    ...
    def test_resume_without_a_token_raises_when_required(self, cfg):
        with patch("eeg_win_stack.tools.tracking.mlflow", MagicMock()):
    ...
    def test_azure_artifact_root_passed_on_creation(self, cfg):
        ...
        mlflow.create_experiment.assert_called_once_with(
            "eeg_win_stack", artifact_location="wasbs://bucket/experiments"
        )
