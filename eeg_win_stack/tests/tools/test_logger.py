...
def test_names_are_reparented_under_the_package_root():
    ...
    assert Logger("io.dataset_builder").name == "eeg_win_stack.io.dataset_builder"
    # __name__ from inside the package is already qualified, so it is left alone.
    assert Logger("eeg_win_stack.models.factory").name == "eeg_win_stack.models.factory"
...
def test_log_file_is_written_and_parents_created(tmp_path):
    ...
    assert log_file.read_text().strip().endswith("eeg_win_stack.pipeline: stage complete")


def test_child_extends_the_parent_name():
    assert get_logger("io").child("labeling").name == "eeg_win_stack.io.labeling"
