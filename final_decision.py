...
BLOCK = [
    ...
    3,
    4,
]  # 0,1,2,3,4,5,6,7   # How many results of several experiments with the first stage model you want to use
HIDDEN_LAYERS = [0]  # depth of hidden layer
HIDDEN_LENGTH = [5]  # length of hidden layer
START_ROW_GAP = [
    [1, 4, 4]
]  # [266,4,3],[412,2,3],[381,1,3],[584,4,4],[544,2,4],[484,1,4]    [1,4,4],[91,1,4],[121,2,4],[231,4,4],[191,2,4],[161,1,4]   #start row, how many row labels, how many rows between probabilities and next labels
            ...
            if use_his:
                if use_hybrid:
                    data.append(
                        np.concatenate(
                            [
                                creat_his(pd_data[pd_valid_lens[i] : pd_valid_lens[i + 1]], length=length),
                                pd_data[pd_valid_lens[i] : pd_valid_lens[i + 1]] + [0] * (20 - valid_lens[-1]),
                            ],
                            axis=0,
                        )
                    )
    ...
    if xgboost:  # Use xgboost as the second stage model
        ...
        for n_time in range(n_repetition):
            ...
            idx_train, idx_test = train_test_split(
                np.arange(len(valid_lens)), random_state=n_time, train_size=train_ratio, shuffle=not fix_testset
            )

            idx_train, idx_valid = train_test_split(idx_train, random_state=n_time, train_size=0.75, shuffle=True)
            ...
            class histogram_model(nn.Module):
                def __init__(
                    self, length=10, use_hybrid=False, hidden_layers=hidden_layers, hidden_length=hidden_length
                ):
                    super().__init__()
                    if use_hybrid:
